				...
				<div id="secondstep" class="secondstep1">
					<div id="selectDiv" style="display: ; margin-top: 10px;">
						<label for="cars">CHOOSE A CENTER:</label> <select id="cars"
							name="center">
							<option value="<%=request.getAttribute("data2")%>"><%=request.getAttribute("data2")%>--<%=request.getAttribute("data5") %></option>
							<%
...
tooltip: {
    ...
    pointFormat: 'Stroke Case: {point.y}'
},
title: {
    text: 'STROKE CASES BY CENTER (<%=request.getAttribute("data2")%>) IN  YEARS',
    align: 'center'
...
series: [{
    data: [
        <% for(int j=0; j<myList.size(); j++) {
        	GraphByCenterYear alltable1 = (GraphByCenterYear)myList.get(j);
        %>
              <%= alltable1.getYearByData() %>,
        <% } %>
    ],
    colorByPoint: true
}]
